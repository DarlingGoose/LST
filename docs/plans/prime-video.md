...
- **U4. Does the ASIN in the path change when Prime auto-advances** to the next
  ...
  `docs/research/prime-video-recon.md` §U4: the playback-resources answer names the item being
...
and a drift guard in `tests/test-playback-site.mjs` asserts that every pattern in
...
from §2.3. Deliverable: `docs/research/prime-video-recon.md`, with the real caption DOM shape,
...
9. **UI, setup and docs** — `options.html`, `popup.js`, `setup.js`/`setup.html`,
   `README.md`, `AGENTS.md`, `PRIVACY.md`, `docs/releases/amo-reviewer-notes.md`, `amo-metadata.json`,
...
- Store copy and screenshots; `amo-metadata.json` gains Prime Video and the
  ...
  `docs/releases/amo-reviewer-notes.md` gains the Prime capture explanation and the permission
...
- **`tests/test-playback-site.mjs`** (new) — the adapter as a table:
