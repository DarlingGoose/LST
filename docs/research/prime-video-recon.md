...
The plan that this document belongs to is `docs/plans/prime-video.md` at the repository
...
- Everything after the capture — parsing, precompute, the transcript sidebar,
  ...
  the earlier refactors promised, and `tests/test-page-hook.mjs` and
  `tests/test-playback-site.mjs` now hold it.
...
`docs/plans/prime-video.md` §9 expected `host_permissions` to stay exactly as it was and
