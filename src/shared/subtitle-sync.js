(() => {
  ...
  function resolveTrackDocument(existingCues, incomingCues, options = {}) {
    ...
    return {
      ...
      reason,
      relationship,
      // Every cue the incoming document holds is a cue the track in use already
      // holds: the document is that track, or a part of it, and not another
      // episode's. The caller asks this when it has to decide what a refused
      // document could still be good for — a document that is the track being
      // watched belongs to the episode that is being watched and to no other one.
      incomingWithinExisting:
        existing.length > 0 && incoming.length > 0 && incomingOnlyCueCount === 0,
