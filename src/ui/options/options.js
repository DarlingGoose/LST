...
if (!settingsSchema) throw new Error("settings-schema.js must load before options.js");
...
let currentPlayerHealthSnapshot = null;
...
function renderPlayerControlsVisibility() {
  const visible = $("showQuickPills").checked;
  $("playerControlsVisibilityHelp").textContent = visible
    ? "Shown globally. Individual show visibility settings may still override subtitle layers."
    : "Hidden globally on every service. Use this switch, the popup recovery action, or Reset interface settings to restore them.";
}
...
function playerHealthJson() {
  return JSON.stringify(currentPlayerHealthSnapshot, null, 2);
}

async function loadPlayerHealth() {
  const summary = $("playerHealthSummary");
  const output = $("playerHealthSnapshot");
  const copy = $("copyPlayerHealth");
  summary.textContent = "Looking for an open Netflix or Prime Video player…";
  output.textContent = "Checking open tabs…";
  copy.disabled = true;
  currentPlayerHealthSnapshot = null;

  let tabs = [];
  try {
    tabs = await ext.tabs.query({});
  } catch (error) {
    throw new Error(`Could not inspect open player tabs: ${error.message}`);
  }
    ...
    tabs
      ...
      .map(async (tab) => {
        const response = await ext.tabs.sendMessage(tab.id, {
          type: "GET_TROUBLESHOOTING_STATUS",
        });
  ...
  currentPlayerHealthSnapshot =
    snapshots.find((snapshot) => snapshot.session?.active) ||
    snapshots.find((snapshot) => snapshot.site?.playbackPage) ||
    snapshots[0] ||
    null;

  if (!currentPlayerHealthSnapshot) {
    summary.textContent = "No compatible player runtime answered.";
    output.textContent =
      "Open or reload a Netflix or Prime Video player tab, start the episode, then refresh this snapshot.";
    return;
  }

  const snapshot = currentPlayerHealthSnapshot;
  const service = snapshot.site?.id && snapshot.site.id !== "none"
    ? snapshot.site.id
    : "player";
  summary.textContent = snapshot.session?.active
    ? `Live ${service} session · captured ${snapshot.track?.cueCount || 0} cues · ` +
      `${snapshot.cache?.eligibilityReason || "cache state unknown"}`
    : `${service} content runtime found, but no active player session is running.`;
  output.textContent = playerHealthJson();
  copy.disabled = false;
}
...
async function load() {
  ...
  renderPlayerControlsVisibility();
  ...
  loadPlayerHealth().catch((error) => {
    $("playerHealthSummary").textContent = "Could not read player health.";
    $("playerHealthSnapshot").textContent = error.message;
  });
...
$("showQuickPills").addEventListener("change", renderPlayerControlsVisibility);

$("resetInterface").addEventListener("click", async () => {
  if (!confirm(
    "Reset interface settings and all per-show visibility overrides? Translation caches and imported subtitle files will be kept.",
  )) return;
  const button = $("resetInterface");
  button.disabled = true;
  try {
    await runtimeMessage({ type: "RESET_INTERFACE_SETTINGS" });
    await pushSettingsToOpenTabs();
    setStatus("Interface settings restored. Reloading this page…", "success");
    setTimeout(() => location.reload(), 250);
  } catch (error) {
    button.disabled = false;
    setStatus(`Could not reset interface settings: ${error.message}`, "error");
  }
});
...
$("openSetup").addEventListener("click", () => {
  try {
    ext.tabs.create({ url: ext.runtime.getURL("ui/setup/setup.html") })
...
$("refreshPlayerHealth").addEventListener("click", () => {
  loadPlayerHealth().catch((error) => {
    $("playerHealthSummary").textContent = "Could not read player health.";
    $("playerHealthSnapshot").textContent = error.message;
    setStatus(`Could not read player health: ${error.message}`, "error");
  });
});

$("copyPlayerHealth").addEventListener("click", async () => {
  if (!currentPlayerHealthSnapshot) return;
  try {
    await navigator.clipboard.writeText(playerHealthJson());
    setStatus("Redacted player health snapshot copied as JSON.", "success");
  } catch (error) {
    setStatus(`Could not copy player health: ${error.message}`, "error");
  }
});
