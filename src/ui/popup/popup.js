...
if (!settingsSchema) throw new Error("settings-schema.js must load before popup.js");
...
function syncSettingsControls() {
  ...
  $("restorePlayerControls").hidden = state.settings.showQuickPills !== false;
...
$("restorePlayerControls").addEventListener("click", async () => {
  await saveQuickSettings(
    { showQuickPills: true },
    "In-player controls restored. Return to the player to use them.",
  );
});
...
$("importSubtitles").addEventListener("click", () => {
  try {
    const created = ext.tabs.create({
      url: ext.runtime.getURL("ui/options/options.html#import"),
    });
...
if (hasExtensionApi) {
  ...
  state.settings = settingsSchema.defaults();
