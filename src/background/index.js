...
if (!settingsSchema) throw new Error("settings-schema.js must load before background.js");
...
function normalizedShowSettings(value) {
  return settingsSchema.normalizeShowOverrides(value);
}

const DEFAULTS = settingsSchema.DEFAULTS;
const SETTING_KEYS = settingsSchema.STORAGE_KEYS;

function migrateSettings(stored) {
  return settingsSchema.normalizeStored(stored).settings;
}

ext.runtime.onInstalled.addListener(async (details) => {
  const current = await ext.storage.local.get(SETTING_KEYS);
  const missing = settingsSchema.installPatch(current);
  ...
  try {
    await ext.tabs.create({ url: ext.runtime.getURL("ui/setup/setup.html") });
  } catch (error) {
...
async function getSettingsRecord() {
  return settingsSchema.normalizeStored(await ext.storage.local.get(SETTING_KEYS));
}
  ...
  (async () => {
    switch (message?.type) {
      case "GET_SETTINGS": {
        ...
        sendResponse({ ok: true, settings: record.settings, sources: record.sources });
        return;
      }

      case "SAVE_SETTINGS": {
        const requested = { ...(message.settings || {}) };
        if (requested.provider && !["ollama", "deepseek", "gemini"].includes(requested.provider)) {
          throw new Error("Unknown translation provider.");
        }
        for (const keyName of Object.values(PROVIDER_KEYS)) delete requested[keyName];
        ...
        try {
          delete requested[importKeyStorage()];
        } catch {
          // The import module is missing; its key was never written either.
        }
        const patch = settingsSchema.normalizePatch(requested);
        await ext.storage.local.set(patch);
        const legacyKeys = settingsSchema.legacyKeysForPatch(patch);
      ...
      case "RESET_INTERFACE_SETTINGS": {
        await ext.storage.local.set({
          ...settingsSchema.interfaceDefaults(),
          settingsSchemaVersion: settingsSchema.VERSION,
          [SHOW_SETTINGS_KEY]: {},
        });
        const record = await getSettingsRecord();
        sendResponse({ ok: true, settings: record.settings, sources: record.sources });
      ...
      case "OPEN_OPTIONS": {
        ...
        if (hash) {
          await ext.tabs.create({ url: ext.runtime.getURL(`ui/options/options.html${hash}`) });
        } else {
