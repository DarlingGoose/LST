(() => {
  ...
  let settings = settingsSchema.defaults();
  let effectiveSettingSources = Object.fromEntries(
    settingsSchema.SETTING_NAMES.map((name) => [name, "default"]),
  );
  let globalSettingSources = { ...effectiveSettingSources };
  ...
  function recordPlayerSessionTransition(state, reason) {
    const transition = {
    ...
    logDiagnostic("info", "player", `session-${state}`, {
      ...
      playerPresenceReason: playerPresence().reason || "unspecified",
    });
  }
  ...
  async function loadSettings() {
    try {
      // These preferences already live in extension-local storage. Reading them
      // through the background worker made playback startup depend on that
      // worker waking: opening the toolbar popup woke it, but by then Prime's
      // one-shot playback-resources capture could already be too old to adopt.
      // Keep the message fallback for an older/incomplete installation whose
      // local store has not been initialized yet.
      let stored = await ext.storage.local.get(settingsSchema.STORAGE_KEYS);
      if (!stored || !Object.keys(stored).length) {
        ...
        const resolved = settingsSchema.normalizeStored(stored);
        settings = resolved.settings;
        effectiveSettingSources = resolved.sources;
      }
      globalShowDefaults = Object.fromEntries(
        [...SHOW_SETTING_NAMES].map((key) => [key, settings[key]]),
      );
      globalSettingSources = { ...effectiveSettingSources };
    } catch (error) {
  ...
  async function refreshShowSettings({ force = false } = {}) {
    ...
    for (const name of SHOW_SETTING_NAMES) {
      ...
      effectiveSettingSources[name] = typeof overrides[name] === "boolean"
        ? "show"
        : globalSettingSources[name] || "default";
    }
  ...
  function saveShowSetting(name, value) {
    ...
    appliedShowSettingsKey = key;
    effectiveSettingSources[name] = "show";
  ...
  function ensureOverlay({ applyAppearance = true } = {}) {
    ...
    syncOverlayHost();
    updateDebugPanel();
    if (applyAppearance) applySubtitleAppearance();
    updateOverlayPanelVisibility();
    return overlay;
  }
  ...
  function updateQuickPills() {
    ...
    if (panelStatus) {
      panelStatus.textContent = currentStatus.pausedCacheBlocker ||
  ...
  function logQuickPillsHealth() {
    if (!quickPillsPanel) return;
    const details = {
      configuredVisible: settings.showQuickPills !== false,
      connected: Boolean(quickPillsPanel.isConnected),
      displayed: quickPillsPanel.style.display !== "none",
      compact: quickPillsPanel.dataset.compact === "true",
      menuOpen: Boolean(quickPillsMenu && !quickPillsMenu.hidden),
    };
    const signature = JSON.stringify(details);
    if (signature === lastQuickPillsHealthSignature) return;
    lastQuickPillsHealthSignature = signature;
    logDiagnostic("info", "controls", "player-controls-state-changed", details);
  }
  ...
  function bindQuickPills() {
    ...
    quickPillsPanel.addEventListener("change", (event) => {
      ...
      const save = SHOW_SETTING_NAMES.has(key)
        ...
        : (() => {
            effectiveSettingSources[key] = "global";
            globalSettingSources[key] = "global";
            return runtimeMessage({
              type: "SAVE_SETTINGS",
  ...
  function validateTimedTrackAgainstRendering(video, renderedText) {
    ...
    if (!renderedText) {
      // A gap in what the player draws says nothing about which track the player
      // is playing: it is the absence of a line, not a line that agrees with the
      // track. A track the player has contradicted therefore stays contradicted
      // until a line the track holds is drawn again — otherwise the previous
      // episode's cues come back on screen at every pause between lines, which is
  ...
  function pausedCacheDecision(video) {
    ...
    const input = {
      ...
      cacheWhilePaused: settings.cacheWhilePaused !== false,
      translationPaused: settings.translationPaused === true,
      model: settings.model,
      cueCount: cues.length,
      needsTranslation: cues.length ? trackNeedsTranslation() : true,
      precomputing: precomputeInProgress,
      complete,
    };
    return globalThis.LSTPlaybackPolicy?.pausedCacheDecision?.(input) || {
      allowed: Boolean(
        input.videoPaused &&
        input.cacheWhilePaused &&
        !input.translationPaused &&
        input.model &&
        input.cueCount &&
  ...
  function updatePausedCacheBlocker(video) {
    const previousReason = currentStatus.pausedCacheReason;
    const previousBlocker = currentStatus.pausedCacheBlocker;
    if (!video?.paused || pausedCachingPromise) {
      currentStatus.pausedCacheReason = "";
      currentStatus.pausedCacheBlocker = "";
      const changed = Boolean(previousReason || previousBlocker);
      if (changed) {
        logDiagnostic("info", "cache", "paused-cache-eligibility-changed", {
          allowed: Boolean(pausedCachingPromise),
          reason: pausedCachingPromise ? "caching" : "not-paused",
        });
      }
      return changed;
    }
    const decision = pausedCacheDecision(video);
    currentStatus.pausedCacheReason = decision.reason;
    currentStatus.pausedCacheBlocker = decision.allowed
      ? ""
      : globalThis.LSTPlaybackPolicy?.describePausedCacheDecision?.(decision) ||
        "Paused caching is unavailable.";
    const changed = previousReason !== currentStatus.pausedCacheReason ||
      previousBlocker !== currentStatus.pausedCacheBlocker;
    if (changed) {
      logDiagnostic(
        decision.allowed ? "info" : "warning",
        "cache",
        "paused-cache-eligibility-changed",
        { allowed: Boolean(decision.allowed), reason: decision.reason || "unspecified" },
      );
    }
    return changed;
  }
  ...
  function startPausedCaching(video) {
    if (pausedCachingPromise) return;
    const decision = pausedCacheDecision(video);
    if (!decision.allowed) {
      updatePausedCacheBlocker(video);
      updateQuickPills();
      return;
    }
  ...
  async function runPausedCaching(video) {
    ...
    while (
      ...
      const cachedBefore = knownCachedKeys.size;
      const result = await translateCues(batch);
      for (const failure of result.failures) {
        if (failure.id) pausedCacheFailedKeys.add(failure.id);
      }
      if (knownCachedKeys.size === cachedBefore && !result.failures.length) {
        // A provider returning neither translations nor failures must not spin
        // forever over the same batch while the video is paused.
        for (const cue of batch) pausedCacheFailedKeys.add(cueKey(cue));
        currentStatus.lastError =
          "Paused caching stopped because the provider returned no translations.";
        setStatus(currentStatus.lastError, true, "error");
        return;
      }
  ...
  async function playbackLoop() {
    ...
    const pausedCacheBlockerChanged = updatePausedCacheBlocker(video);

    if (!cues.length) {
      if (pausedCacheBlockerChanged) updateQuickPills();
      requestAnimationFrame(playbackLoop);
      return;
    }
    ...
    if (!importedNow) {
      ...
      // A player that draws no line at all — a viewer who has the service's own
      // captions off — says nothing about which track it holds. What is left is
      // the clock, and it says the item restarted when a document captured deep
      // into the item is offered while the player is back at the item's start.
      if (!renderedText) noticeRestartedItem(video);
      if (
        renderedObservation.stable &&
  ...
  async function handleFallbackRenderedSubtitle() {
    ...
    // A full timed-text document is not guaranteed to arrive before Netflix
    // draws the first line (and sometimes does not arrive at all). The visible
    // line is still enough to avoid translating English to English. Make that
    // decision before requiring a model so native same-language captions also
    // recover correctly while translation is paused or unconfigured.
    if (!cues.length) {
      const needsTranslation = trackNeedsTranslation(text);
      if (!needsTranslation) {
        const alreadyKnown =
          sameLanguageTargetAcrossEpisode === normalizedTargetLanguage();
        sameLanguageTargetAcrossEpisode = normalizedTargetLanguage();
        ...
        currentStatus.playbackMode = "native captions (target language)";
        currentStatus.lastTranslatedText = "";
        removeRenderedSubtitlesBySource("dom", "same-language-native-captions");
        removeRenderedSubtitlesBySource("timed", "same-language-native-captions");
        applySubtitleAppearance();
        if (!alreadyKnown) {
          setStatus(
            `Using ${siteName()}'s native ${settings.targetLanguage} subtitles.`,
          ...
          logDiagnostic("info", "translation", "translation-skipped", {
            reason: playbackLanguages().subtitle
              ? "subtitle-language-matches-target"
              : "rendered-subtitle-script-matches-target",
            sourceLanguage: playbackLanguages().subtitle || "inferred",
            targetLanguage: settings.targetLanguage,
          });
        }
        return;
      ...
      // A carried decision only bridges the short player teardown between
      // episodes. A visible line that needs translation supersedes it.
      if (sameLanguageTargetAcrossEpisode === normalizedTargetLanguage()) {
        sameLanguageTargetAcrossEpisode = "";
        applySubtitleAppearance();
      }
    }

    if (!settings.model || settings.translationPaused) return;
  ...
  async function acceptSubtitleDocument(url, text, details = {}) {
    ...
    if (!decision.accept) {
      ...
      // A document the module says is the track already in use — the same cues,
      // or a part of them — belongs to the episode being watched and to no
      // other one, so it is not kept as a candidate for a later episode. Keeping
      // it is how the episode that ends hands its own subtitles to the episode
  ...
  function holdCapturedDocument(payload) {
    ...
    heldCapturedDocument = {
      ...
      // Which episode LST was working with when this document was captured, and
      // which item the address named at the same moment. They are two different
      // questions — on Prime Video the episode is the listing's own id while the
      // address usually names the series — so they are recorded separately and
      // asked separately when the document is offered to another episode.
      videoId: getVideoId(),
      pageVideoId: pageVideoId(),
      // Where the player was in the item when the document was captured, so a
      // document held for the next episode can be told apart from one fetched
      // ahead of a switch that has not happened: only the item restarting moves
      // the clock back.
  ...
  // own capture.
  //
  // The ownership question is asked of the two facts recorded with the document,
  // and only one of them has to still agree: the episode LST was working with,
  // or the item the address named. Either one says the capture was made for what
  // is being played now — the listing names the episode before it hands over the
  // document, so a document held for the next episode already names the next
  // episode, while one held before any listing named it is still the document
  // the address it was captured on belongs to. When neither agrees, both have
  // moved since the capture and the document belongs to the episode LST has
  ...
  function adoptHeldCapturedDocument(reason) {
    ...
    if (
      ...
      logDiagnostic("info", "track", "held-document-other-episode", {
        ...
        capturedPageVideoId: held.pageVideoId,
        pageVideoId: currentPageVideoId,
      });
      return false;
    }
  ...
  // The other piece of evidence, for a player that draws no line at all: a
  // viewer who has the service's own captions off. Then nothing the player draws
  // can say which document belongs to this episode, and the clock says it
  // instead. The clock only ever moves forward inside one item, so a document
  // captured while the player was deep into the item and offered while the player
  // is back at the item's start belongs to the item that started after the
  // capture. Two things follow, and both are needed: the track in use was trusted
  // for the item that has ended, so it stops being trusted — that is what stops
  ...
  function handlePageCaptureMessage(message) {
    if (message?.source !== SOURCE) return;
    if (message.delivery === "replay") {
      logDiagnostic("info", "track", "capture-bridge-replayed", {
        captureType: message.type || "unknown",
        siteId: message.payload?.site || currentSiteId() || "unknown",
      });
    }
    ...
    if (message?.type === "EPISODE_IDENTITY") {
      applySiteEpisodeIdentity(message.payload);
      return;
    }

    if (message?.type === "SUBTITLE_DOCUMENT") {
      lastCaptureSummary = {
        at: new Date().toISOString(),
        event: "subtitle-document",
        reason: message.payload?.reason || message.payload?.capture || "captured",
        trackCount: 1,
        language: message.payload?.language || "unknown",
      };
    } else if (message?.type === "SUBTITLE_CAPTURE") {
      lastCaptureSummary = {
        at: new Date().toISOString(),
        event: message.payload?.event || "note",
        reason: message.payload?.reason || "unspecified",
        trackCount: Number(message.payload?.trackCount) || 0,
        language: message.payload?.language || "unknown",
      };
    }
    ...
    if (!playbackActive || !isWatchPage()) {
      ...
      if (message?.type === "SUBTITLE_DOCUMENT") holdCapturedDocument(message.payload);
      return;
    }
    ...
    // wait.
    if (message?.type === "SUBTITLE_CAPTURE") {
    ...
    if (message?.type !== "SUBTITLE_DOCUMENT") return;

    handleCapturedDocument(message.payload);
  }

  if (globalThis.LSTCaptureBridge?.subscribe) {
    globalThis.LSTCaptureBridge.subscribe(handlePageCaptureMessage);
  } else {
    // Development and older-package fallback. The release manifest loads the
    // bridge first, but a directly evaluated content.js remains usable.
    window.addEventListener("message", (event) => {
      if (event.source !== window) return;
      handlePageCaptureMessage(event.data);
    });
  }

  // A copyable report for Settings. It deliberately reconstructs a small
  // allow-list instead of spreading runtime objects: currentStatus also holds
  // subtitle text and raw provider output, while captured documents hold signed
  // URLs. Neither belongs in a report a viewer may share while troubleshooting.
  function troubleshootingSnapshot() {
    const video = activeVideo();
    const presence = playerPresence();
    const detected = detectedSite();
    const languages = playbackLanguages();
    const progress = loadProgress();
    const pausedDecision = pausedCacheDecision(video);
    const identityApi = episodeIdentity();
    const videoId = getVideoId();
    const extensionRoot = String(ext.runtime.getURL?.("") || "");
    const settingRecord = (name, value = settings[name]) => ({
      value,
      source: effectiveSettingSources[name] || "default",
    });
    let needsTranslation = null;
    if (cues.length) {
      try {
        needsTranslation = trackNeedsTranslation();
      } catch {}
    }
    let captureBridge = { available: false };
    try {
      const bridgeSnapshot = globalThis.LSTCaptureBridge?.snapshot?.();
      if (bridgeSnapshot) captureBridge = { available: true, ...bridgeSnapshot };
    } catch {}
    ...
    return {
      ...
      extension: {
        ...
        browserFamily: extensionRoot.startsWith("moz-extension:")
          ? "firefox"
          : extensionRoot.startsWith("chrome-extension:")
            ? "chromium"
            : "unknown",
      },
      site: {
        id: detected.siteId || "none",
        adapterReason: detected.reason || "unspecified",
        matchPattern: detected.matchPattern || "none",
        playbackPage: isWatchPage(),
        playerPresent: Boolean(presence.present),
      ...
      settings: {
        enabled: settingRecord("enabled", settings.enabled !== false),
        siteEnabled: settingRecord("enabledSites", siteEnabled(currentSiteId())),
        showQuickPills: settingRecord("showQuickPills", settings.showQuickPills !== false),
        autoMinimizeControls: settingRecord(
          "autoMinimizeControls",
          settings.autoMinimizeControls !== false,
        ),
        controlsMinimizeDelaySeconds: settingRecord(
          "controlsMinimizeDelaySeconds",
          Number(settings.controlsMinimizeDelaySeconds) || 2,
        ),
        translationPaused: settingRecord(
          "translationPaused",
          settings.translationPaused === true,
        ),
        cacheWhilePaused: settingRecord("cacheWhilePaused", settings.cacheWhilePaused !== false),
        autoTranslateAhead: settingRecord(
          "autoTranslateAhead",
          settings.autoTranslateAhead !== false,
        ),
        showTranslated: settingRecord("showTranslated", settings.showTranslated !== false),
        hideNativeSubtitles: settingRecord(
          "hideNativeSubtitles",
          settings.hideNativeSubtitles !== false,
        ),
        showTranscriptSidebar: settingRecord(
          "showTranscriptSidebar",
          settings.showTranscriptSidebar === true,
        ),
        targetLanguage: settingRecord("targetLanguage", settings.targetLanguage || ""),
        modelConfigured: settingRecord("model", Boolean(settings.model)),
      },
      playerUi: {
        overlayConnected: Boolean(overlay?.isConnected),
        hudConnected: Boolean(hud?.isConnected),
        hudDisplayed: Boolean(hud?.isConnected && hud.style.display !== "none"),
        pillConnected: Boolean(quickPillsPanel?.isConnected),
        pillDisplayed: Boolean(
          quickPillsPanel?.isConnected && quickPillsPanel.style.display !== "none"
        ),
        pillCompact: quickPillsPanel?.dataset.compact === "true",
        pillMenuOpen: Boolean(quickPillsMenu && !quickPillsMenu.hidden),
      },
      track: {
        kind: cueTrackKind,
        captured: Boolean(currentStatus.captured),
        cueCount: cues.length,
        imported: trackIsImported(),
        sourceLanguage: capturedTrackLanguage || languages.subtitle || "unknown",
        audioLanguage: languages.audio || "unknown",
        needsTranslation,
        translationReason: needsTranslation == null
          ? "no-track"
          : needsTranslation
        ...
        episodeIdentity: {
          ...
          source: siteEpisodeIdentity?.videoIdSource || "page",
        },
      },
      cache: {
        translatedCueCount: progress.cached,
        totalCueCount: progress.total,
        progressVisible: progress.visible,
        progressPercent: progress.percent,
        cachedAheadSeconds: currentStatus.cachedAheadSeconds,
        eligibilityAllowed: Boolean(pausedDecision.allowed),
        eligibilityReason: pausedDecision.reason || "unspecified",
        blocker: currentStatus.pausedCacheBlocker || "",
        pausedCaching: Boolean(pausedCachingPromise),
        precomputing: Boolean(currentStatus.precomputing || precomputeInProgress),
      },
      translation: {
        requestState: currentStatus.requestState,
        playbackMode: currentStatus.playbackMode,
        failedCueCount: currentStatus.failedCount,
        lastErrorPresent: Boolean(currentStatus.lastError),
      },
    };
  }
  ...
  ext.runtime.onMessage.addListener((message, sender, sendResponse) => {
    (async () => {
      switch (message?.type) {
        case "GET_TROUBLESHOOTING_STATUS": {
          refreshCacheCoverage(activeVideo()?.currentTime);
          updatePausedCacheBlocker(activeVideo());
          sendResponse({ ok: true, snapshot: troubleshootingSnapshot() });
          return;
        }
  ...
  function stopPlayback(reason = "playback-ended") {
    ...
    diagnosticFlushTimer = null;
    diagnosticEvents = [];
    lastQuickPillsHealthSignature = "";
    lastFallbackText = "";
    renderedSubtitleCandidate = "";
    cues = [];
    cueSourceUrl = "";
    capturedTrackLanguage = "";
    // Netflix briefly removes and recreates its player while advancing to the
    // next episode. Keep the confirmed same-language decision through that
    // teardown; the first visible line or captured track re-evaluates it.
    cueVideoId = "";
    cueTrackKind = "none";
    importedTrack = null;
  ...
  let startupVisibilityLoaded = false;

  async function ensureStartupVisibilityLoaded() {
    if (startupVisibilityLoaded) return;
    try {
      ...
      if (typeof stored.enabled === "boolean") {
        ...
        effectiveSettingSources.enabled = "global";
      }
      if (stored.enabledSites && typeof stored.enabledSites === "object") {
        settings.enabledSites = settingsSchema.normalizeValue("enabledSites", stored.enabledSites);
        effectiveSettingSources.enabledSites = "global";
      }
      if (typeof stored.showQuickPills === "boolean") {
        settings.showQuickPills = stored.showQuickPills;
        effectiveSettingSources.showQuickPills = "global";
      }
    } catch (error) {
      console.warn("[LST] Could not load control visibility settings:", error);
    }
    startupVisibilityLoaded = true;
  }
  ...
  async function syncPlaybackRoute() {
    ...
    // Read the three visibility switches directly from local storage, then draw
    // the default-on pill before asking the background worker for all settings.
    // A sleeping worker can take time to wake (and opening the toolbar popup
    // wakes it too), but the player controls must not depend on that popup.
    // Appearance changes such as hiding native captions still wait for the full
    // settings response.
    await ensureStartupVisibilityLoaded();
    if (
      settings.enabled &&
