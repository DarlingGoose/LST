...
const source = await fs.readFile(new URL("../src/background/index.js", import.meta.url), "utf8");
vm.runInContext(
...
vm.runInContext(
  await fs.readFile(new URL("../src/shared/subtitle-import.js", import.meta.url), "utf8"),
  context,
...
// Resetting the interface is deliberately narrower than resetting LST data.
// It restores the hidden pill and clears per-show visibility choices while
// leaving translation caches and imported files byte-for-byte alone.
const cacheSentinel = { entries: { cue: "cached" } };
const importSentinel = { episode: { text: "stored locally" } };
values["translationCache:keep-after-interface-reset"] = cacheSentinel;
values[context.LSTSubtitleImport.IMPORTED_TRACKS_KEY] = importSentinel;
values.showQuickPills = false;
await send({ type: "RESET_INTERFACE_SETTINGS" });
assert.equal(values.showQuickPills, true);
assert.deepEqual(JSON.parse(JSON.stringify(values.showSettings)), {});
assert.equal(values["translationCache:keep-after-interface-reset"], cacheSentinel);
assert.equal(values[context.LSTSubtitleImport.IMPORTED_TRACKS_KEY], importSentinel);
delete values["translationCache:keep-after-interface-reset"];
delete values[context.LSTSubtitleImport.IMPORTED_TRACKS_KEY];
...

{
  const optionsSource = await fs.readFile(
    new URL("../src/ui/options/options.js", import.meta.url),
    "utf8",
  );
