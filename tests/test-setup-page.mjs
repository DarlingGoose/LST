...
assert.match(html, /<link rel="stylesheet" href="\.\.\/options\/options\.css">/);
...
assert.match(background, /ext\.tabs\.create\(\{ url: ext\.runtime\.getURL\("ui\/setup\/setup\.html"\) \}\)/);
assert.match(settingsSchema, /setupCompleted: \{ default: false, type: "boolean" \}/);
assert.match(html, /<script src="\.\.\/\.\.\/shared\/settings-schema\.js"><\/script>/);
...
assert.equal(JSON.parse(packageJson).scripts["check:syntax"], "node scripts/check-syntax.mjs");
