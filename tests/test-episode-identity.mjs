...
// files that read it.
{
  ...
  assert.ok(backgroundScripts.includes("shared/episode-identity.js"));
  assert.ok(
    backgroundScripts.indexOf("shared/episode-identity.js") <
      backgroundScripts.indexOf("background/index.js"),
  );
  const contentScripts = (manifest.content_scripts || [])
    .filter((entry) => entry.world !== "MAIN")
    .flatMap((entry) => entry.js || []);
  assert.ok(contentScripts.includes("shared/episode-identity.js"));
  assert.ok(
    contentScripts.indexOf("shared/episode-identity.js") < contentScripts.indexOf("content/index.js"),
  );
  ...
  assert.match(
    backgroundSource,
    /importScripts\([\s\S]{0,300}"\.\.\/shared\/episode-identity\.js"/,
  );
  assert.ok(
    backgroundSource.indexOf('"../sites/playback-site.js"') <
      backgroundSource.indexOf('"../shared/episode-identity.js"'),
    "playback-site.js must be imported before episode-identity.js",
  );
  assert.doesNotMatch(backgroundSource, /isGenericCacheName/);
  for (const page of ["../src/ui/popup/popup.html", "../src/ui/options/options.html", "../src/ui/setup/setup.html"]) {
    const html = await fs.readFile(new URL(page, import.meta.url), "utf8");
    assert.match(
      html,
      /<script src="\.\.\/\.\.\/sites\/playback-site\.js"><\/script>/,
