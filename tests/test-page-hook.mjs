  ...
  context = vm.createContext({
    ...
    postMessage(message) {
      ...
      for (const handler of windowListeners.get("message") || []) {
        handler({ source: windowRef, data: message });
      }
    },
  });
  windowRef = vm.runInContext("globalThis.window = globalThis", context);
...
// A subtitle response can beat the isolated content runtime at document_start.
// The page hook must retain it and replay it when that receiver announces
// itself; after acknowledgement, another ready message must not replay it.
for (const fixture of [
  {
    name: "Netflix",
    href: "https://www.netflix.com/watch/80100172",
    async capture(harness) {
      await harness.fetch(
        "https://www.netflix.com/subtitle.vtt",
        "text/vtt",
        VTT,
  ...
  const harness = createPageHarness(fixture.href);
  await fixture.capture(harness);
  const first = harness.published().at(-1);
  assert.ok(first?.eventId, `${fixture.name}: a retained capture has an event id`);
  const beforeReplay = harness.published().length;
  harness.consumerReady();
  assert.equal(
    harness.published().length,
    beforeReplay + 1,
    `${fixture.name}: an early capture is replayed when the consumer becomes ready`,
  );
  harness.acknowledge(first.eventId);
  harness.consumerReady();
  assert.equal(
    harness.published().length,
    beforeReplay + 1,
    `${fixture.name}: an acknowledged capture is not replayed again`,
  );
}

// The isolated bridge also queues before content.js subscribes, which closes
// the other half of the startup ordering race.
{
  const listeners = [];
  const posted = [];
  let bridgeContext;
  bridgeContext = vm.createContext({
    setInterval: () => 1,
    clearInterval() {},
    addEventListener(type, handler) {
  ...
  const bridgeWindow = vm.runInContext("globalThis.window = globalThis", bridgeContext);
  vm.runInContext(captureBridgeSource, bridgeContext, {
    filename: "capture-bridge.js",
  });
  const early = {
    source: "lst-local-subtitle-translate",
    channel: "page-capture",
    type: "SUBTITLE_DOCUMENT",
    eventId: "early-netflix-track",
    payload: { site: "netflix", url: "https://www.netflix.com/subtitle.vtt", text: VTT },
  };
  for (const listener of listeners) {
    listener({ source: bridgeWindow, data: early });
  }
  const delivered = [];
  bridgeContext.LSTCaptureBridge.subscribe((message) => delivered.push(message));
  assert.deepEqual(delivered.map((message) => message.eventId), ["early-netflix-track"]);
  assert.ok(
    posted.some(
      (message) =>
        message.type === "CAPTURE_EVENT_ACK" &&
        message.eventId === "early-netflix-track",
    ),
    "the bridge acknowledges only after content subscribes",
  );
  const bridgeSnapshot = bridgeContext.LSTCaptureBridge.snapshot();
  assert.deepEqual(
    {
      receivedCount: bridgeSnapshot.receivedCount,
      queuedCount: bridgeSnapshot.queuedCount,
      deliveredCount: bridgeSnapshot.deliveredCount,
      acknowledgedCount: bridgeSnapshot.acknowledgedCount,
      subscriberReady: bridgeSnapshot.subscriberReady,
    },
    {
      receivedCount: 1,
      queuedCount: 0,
    ...
    "the diagnostic snapshot reports delivery health as counts",
  );
  assert.equal(
    JSON.stringify(bridgeSnapshot).includes("I don't know"),
    false,
    "the bridge snapshot never exposes captured subtitle text",
  );
}
