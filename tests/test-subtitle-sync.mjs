...
const syncSource = await fs.readFile(
  new URL("../src/shared/subtitle-sync.js", import.meta.url),
  "utf8",
);
const episodeIdentitySource = await fs.readFile(
  new URL("../src/shared/episode-identity.js", import.meta.url),
  "utf8",
);
...
const settingsSchemaSource = await fs.readFile(
  new URL("../src/shared/settings-schema.js", import.meta.url),
  "utf8",
);
const subtitleImportSource = await fs.readFile(
  new URL("../src/shared/subtitle-import.js", import.meta.url),
  "utf8",
);
const coordinatorSource = await fs.readFile(
  new URL("../src/shared/translation-coordinator.js", import.meta.url),
  "utf8",
);
const translationContextSource = await fs.readFile(
  new URL("../src/shared/translation-context.js", import.meta.url),
  "utf8",
);
...
// Whether the incoming document holds anything of its own, which is how a caller
// tells a document that *is* the track in use from one that is something else —
// another episode's track, or another track of this episode. A document that is
// the track in use belongs to the episode being watched and to no other one, so
// it is not a candidate for the episode that follows.
assert.equal(
  decide(trackA, cloneTrack(trackA), { trustExisting: true }).incomingWithinExisting,
  true,
  "the same cues are the track in use",
);
assert.equal(
  decide(trackA, trackA.slice(0, 2), { trustExisting: true }).incomingWithinExisting,
  true,
  "part of the track in use is still the track in use",
);
assert.equal(
  decide(trackA, [...cloneTrack(trackA), { start: 40, end: 41, text: "Extra." }], {})
    .incomingWithinExisting,
  false,
  "a document with a line of its own is not the track in use",
);
assert.equal(
  decide(trackA, otherTrack, {}).incomingWithinExisting,
  false,
  "another track's cues are not the track in use",
);
assert.equal(
  decide([], trackA, {}).incomingWithinExisting,
  false,
  "with no track in use there is nothing for a document to be within",
);
...
const contentSource = await fs.readFile(
  new URL("../src/content/index.js", import.meta.url),
  "utf8",
);
...
// A document handed to content.js directly is parsed, judged and stored
// asynchronously, and diagnostics are buffered for half a second before they
// are flushed: this is the wait a test uses when it does not go through the
// harness's own `capture()` helper.
...
function createHarness(
  vtt = CUE_TRACK,
  { site = "netflix", settings = {}, player = null } = {},
) {
  ...
  // A real player is not in the page when the content script first runs: a Prime
  // Video detail page mounts its player and starts it later, in the same
  // document. `player: { mounted: false, started: false }` is that page, and
  // `mountPlayer()` is the viewer pressing play.
  const playerState = {
    mounted: player ? player.mounted !== false : true,
    started: player ? player.started !== false : true,
  };
  const video = {
    currentTime: 0,
    paused: false,
    get duration() {
      return playerState.started ? 1200 : NaN;
    },
    get readyState() {
      return playerState.started ? 4 : 0;
    },
    playbackRate: 1,
    addEventListener() {},
    removeEventListener() {},
  ...
  const document = {
    ...
    querySelector(selector) {
      ...
      if (selector === ".atvwebplayersdk-player-container") {
        return site === "primevideo" && playerState.mounted ? primePlayer : null;
      }
  ...
  vm.runInContext(settingsSchemaSource, context, {
  ...
  return {
    ...
    nativeSubtitlesHidden() {
      return documentElement.classList.contains("lst-hide-native-subtitles");
    },
    ...
    // The player mounts and starts inside the document that is already open,
    // which is what a Prime Video detail page does when the viewer presses play.
    mountPlayer() {
    ...
    playerMounted() {
      return playerState.mounted && playerState.started;
    },
    // One iteration of the route poll. It is the only thing that notices a
    // player that appeared after the content script first ran, so a page whose
    // player arrives late is covered by stepping it rather than by opening the
    // toolbar popup.
    async routeTick() {
      const tick = intervals.find((entry) => entry.ms === 750)?.callback;
      assert.ok(tick, "the route poll should be running");
      tick();
      ...
      handler({
        ...
        data: {
          ...
          payload: {
            ...
            text: vtt,
            // How the page world tags what it publishes: Prime's documents carry
            // the listing they came from and the language the track declared, and
            // the player is not always at the start of the item when it asks for
            // the next episode's assets.
            site,
...
// Player controls are built from local settings and must not depend on waking
// the background worker (opening the toolbar popup also wakes that worker,
// which previously made the pill appear only after the popup was opened).
{
  const harness = createHarness(CUE_TRACK, { site: "netflix" });
  await harness.start({ captureTrack: false });
  assert.ok(
    harness.elementById("lst-quick-pills"),
    "the in-player controls appear as playback starts",
  );
  assert.equal(
    harness.sentMessageTypes().includes("GET_SETTINGS"),
    false,
    "player startup does not wait for the background worker to read local settings",
  );
}
...
// The next episode's own document is captured while the previous track is still
// loaded, exactly as the page world hands it over — tagged with the listing it
// came from and the language that listing declared. The request it arrived in is
// never repeated, so it has to survive to the change, and the change has to know
// it belongs to the episode now playing: the listing named that episode before it
// handed the document over.
{
  ...
  harness.dispatchDocument({
    url: "https://aiv-cdn.net/ttml/ep1.ja.ttml",
    text: CUE_TRACK,
    site: "primevideo",
  ...
  await harness.capture(NEXT_EPISODE_TRACK, {
    url: "https://aiv-cdn.net/ttml/ep2.ja.ttml",
    capture: "playback-resources",
    language: "ja",
    reason: "preferred-language:ja",
  });
  assert.equal(
    harness.eventsNamed("subtitle-track-kept").length,
    1,
    "the next episode's document is held while the old track is loaded",
  );
  ...
  await harness.frame(1);
  await harness.flushDiagnostics();
  assert.ok(harness.eventsNamed("episode-changed").length, "the change is noticed");

  const adopted = harness.eventsNamed("held-document-adopted");
  assert.equal(adopted.length, 1, "the next episode's own document is adopted");
  assert.equal(adopted[0].details.reason, "episode-changed");
  assert.equal(
    harness.eventsNamed("held-document-other-episode").length,
    0,
    "the document names the episode now playing, so it is not another episode's",
  );
  const accepted = harness.eventsNamed("subtitle-track-accepted").at(-1);
  assert.equal(accepted.details.trackReason, "no-existing-track");
  assert.equal(accepted.details.cueCount, 2);
  ...
  await harness.frame(5.4);
  assert.ok(
    harness.translationRequests().includes("Next episode line."),
    "the next episode translates again",
  );
}

// A track the player has contradicted stays contradicted when the player draws
// nothing at all. Silence is not a line that agrees with the track: treating it
// as one is what puts the previous episode's cues back on screen at every pause
// between lines, which is what watching the old subtitles on the next episode
// looks like.
{
  const harness = createHarness(CUE_TRACK, { site: "primevideo" });
  await harness.start();
  ...
  assert.equal(
    harness.eventsNamed("subtitle-track-accepted").length,
    1,
    "the first episode is playing from its own track",
  );

  // The next episode, with nothing naming it: the line the player draws is not
  // one this track holds.
  await harness.show("Next episode line.", 20.4);
  await harness.frame(20.4);
  await sleep(400);
  await harness.frame(20.4);
  assert.equal(
    harness.eventsNamed("timed-track-mismatch-started").length,
    1,
    "the track the player contradicts is reported",
  );

  // A pause between the player's lines, at a moment this track has a cue of its
  // own: the cue must not come back.
  await harness.show("", 20.4);
  await harness.frame(20.4);
  const status = (await harness.pageMessage({ type: "GET_PAGE_STATUS" })).status;
  assert.equal(
    status.activeCueStart,
    null,
    "the track's own cue is not rendered while the player contradicts it",
  );
  assert.equal(
    status.playbackMode,
    "waiting for a line this track holds",
    "the player is waiting for a line this track holds, and says so",
  );
  await harness.flushDiagnostics();
  assert.equal(
    harness.eventsNamed("timed-track-mismatch-idle").length,
    1,
    "the gap is reported as the mismatch standing still, not as a resolution",
  );
  assert.equal(harness.eventsNamed("timed-track-mismatch-resolved").length, 0);
  assert.equal(harness.eventsNamed("timed-track-mismatch-ended").length, 0);
}

// A document that is the track already in use is not kept as a candidate for a
// later episode. A service re-asks for a title's assets during playback — a
// quality change, a resume — and every answer carries the same document under a
// new signed URL. Keeping that copy is how the episode that ends hands its own
// subtitles to the episode that follows.
{
  const harness = createHarness(CUE_TRACK, { site: "primevideo" });
  await harness.identity({
    reason: "catalog-metadata",
  ...
  // The same title's assets, asked for again: the same document, a new URL.
  await harness.capture(CUE_TRACK, {
    url: "https://aiv-cdn.net/ttml/ep1.ja.ttml?token=again",
    capture: "playback-resources",
    language: "ja",
    reason: "preferred-language:ja",
  });
  const dropped = harness.eventsNamed("held-document-dropped");
  assert.equal(dropped.length, 1, "the copy of the track in use is dropped");
  assert.equal(dropped[0].details.reason, "document-is-current-track");
  ...
  assert.ok(harness.eventsNamed("episode-changed").length, "the change is noticed");
  assert.equal(
    harness.eventsNamed("held-document-adopted").length,
    0,
    "the episode that ended does not hand its own subtitles to the next one",
  );

  // The new episode's own document is what plays it, and none of the old track
  // is left in it.
  await harness.capture(NEXT_EPISODE_TRACK, {
    url: "https://aiv-cdn.net/ttml/ep2.ja.ttml",
    capture: "playback-resources",
  ...
  assert.equal(accepted.details.cueCount, 2);
  const status = (await harness.pageMessage({ type: "GET_PAGE_STATUS" })).status;
  assert.equal(status.cueCount, 2, "the track is the next episode's own cues");
}

// A document captured on an address LST has left, naming an episode it is no
// longer working with, is not the episode now playing: the item's own listing is
// what names the item on the address being played, and the frame that follows
// picks its document up.
{
  const harness = createHarness(CUE_TRACK, { site: "primevideo" });
  await harness.start();
  await harness.show("Let's go.", 20.4);
  ...
  assert.ok(harness.eventsNamed("episode-changed").length, "the address moved");
  const refused = harness.eventsNamed("held-document-other-episode");
  assert.equal(refused.length, 1, "the document belongs to the address LST left");
  assert.equal(refused[0].details.reason, "episode-changed");
  assert.equal(
    harness.eventsNamed("held-document-adopted").length,
    0,
    "it is not handed to the episode now playing",
  );

  // The item being played now has its own listing, and its document is the one
  // that plays it.
  await harness.capture(NEXT_EPISODE_TRACK, {
    url: "https://aiv-cdn.net/ttml/B0B6GZ954Z.ttml?token=fresh",
    capture: "playback-resources",
    language: "ja",
    reason: "preferred-language:ja",
  });
...
// A viewer who has the service's own captions off — which is how a title the
// service does not subtitle in their language is watched at all — leaves the
// player drawing nothing for LST to check its track against. The clock is what
// is left: a document captured while the player was deep into the item, offered
// while the player is back at the item's start, is the document of the item that
// started after the capture, and it takes over from the track that ended.
{
  const harness = createHarness(CUE_TRACK, { site: "primevideo" });
  await harness.start({ captureTrack: false });
  harness.dispatchDocument({
  ...
  // The service asks for the next episode's assets while the first episode is
  // still playing, which is what it does before it switches.
  harness.video.currentTime = 1200;
  await harness.capture(NEXT_EPISODE_TRACK, {
    url: "https://aiv-cdn.net/ttml/ep2.ja.ttml",
    capture: "playback-resources",
    language: "ja",
  ...
  assert.equal(
    harness.eventsNamed("held-document-adopted").length,
    0,
    "assets fetched ahead of a switch that has not happened are not adopted yet",
  );

  // The next episode starts, and the player draws nothing at all.
  await harness.show("", 2);
  await harness.frame(2);
  await harness.flushDiagnostics();
  const withdrawn = harness.eventsNamed("timed-track-not-the-item");
  assert.equal(
    withdrawn.length,
    1,
    "the item restarting withdraws the trust the track had for the item that ended",
  );
  assert.equal(withdrawn[0].details.reason, "item-restarted");
  const adopted = harness.eventsNamed("held-document-adopted");
  assert.equal(adopted.length, 1, "the item restarting is what adopts the document");
  assert.equal(adopted[0].details.reason, "item-restarted-in-held-document");
  const accepted = harness.eventsNamed("subtitle-track-accepted").at(-1);
  assert.equal(
    accepted.details.trackReason,
    "unrelated-track",
    "the document the service fetched is not the track that was playing",
  );
  assert.equal(accepted.details.cueCount, 2);
  const status = (await harness.pageMessage({ type: "GET_PAGE_STATUS" })).status;
  assert.equal(status.cueCount, 2, "the previous episode's cues are gone");
  assert.equal(
    status.activeCueStart,
    null,
    "nothing of the track that ended is rendered",
  );
}
...
// Netflix can render captions before its timed-text response is captured. The
// rendered line must make the same language decision on its own, otherwise LST
// hides the useful native caption while saying that it is waiting for it.
{
  const harness = createHarness(CUE_TRACK, {
    site: "netflix",
    settings: { targetLanguage: "English", autoTranslateAhead: false },
  ...
  assert.deepEqual(
    harness.translationRequests(),
    [],
    "a visible English Netflix caption is not translated to English",
  );
  assert.match(
    harness.elementById("not-status")?.textContent || "",
    /native English subtitles/i,
    "the visible caption clears the waiting state",
  );
  assert.equal(
    harness.nativeSubtitlesHidden(),
    false,
    "the visible same-language caption restores Netflix's native subtitles",
  );
}
...
// browser's own rule for [hidden], which is how that happened here before.
{
  const playerCss = await fs.readFile(
    new URL("../src/content/styles.css", import.meta.url),
    "utf8",
  );
...
// Paused caching is a full-track feature. DOM realtime remains usable without
// one, but the player must explain why it cannot build an episode cache instead
// of sitting indefinitely on a vague Realtime label.
{
  const domOnly = createHarness(CUE_TRACK, { site: "netflix" });
  await domOnly.start({ captureTrack: false });
  domOnly.video.paused = true;
  await domOnly.frame(10);
  const status = await domOnly.pageMessage({ type: "GET_PAGE_STATUS" });
  assert.equal(status.status.pausedCacheReason, "no-full-track");
  assert.match(status.status.pausedCacheBlocker, /DOM realtime mode cannot build/);
  const health = await domOnly.pageMessage({ type: "GET_TROUBLESHOOTING_STATUS" });
  assert.equal(health.snapshot.cache.eligibilityAllowed, false);
  assert.equal(health.snapshot.cache.eligibilityReason, "no-full-track");
  assert.match(health.snapshot.cache.blocker, /DOM realtime mode cannot build/);
}

// With a captured track, pausing playback schedules translation batches and no
// blocker remains.
{
  const captured = createHarness(CUE_TRACK, { site: "netflix" });
  await captured.start({ captureTrack: true });
  const health = await captured.pageMessage({ type: "GET_TROUBLESHOOTING_STATUS" });
  assert.equal(health.ok, true);
  assert.equal(health.snapshot.extension.version, "0.0.0-test");
  assert.equal(health.snapshot.site.id, "netflix");
  assert.equal(health.snapshot.session.active, true);
  assert.equal(health.snapshot.track.kind, "captured");
  assert.equal(health.snapshot.track.cueCount, 3);
  assert.equal(health.snapshot.playerUi.pillConnected, true);
  assert.equal(health.snapshot.settings.showQuickPills.value, true);
  const healthJson = JSON.stringify(health.snapshot);
  for (const privateValue of ["I don't know.", "Let's go.", "subtitle.vtt"]) {
    assert.equal(
      healthJson.includes(privateValue),
      false,
      `the troubleshooting snapshot excludes ${privateValue}`,
    );
  }
  for (const forbiddenKey of ["lastTranslatedText", "lastOllamaRaw", "sourceUrl"]) {
    assert.equal(
      healthJson.includes(forbiddenKey),
      false,
      `the troubleshooting snapshot excludes ${forbiddenKey}`,
    );
  }
  captured.video.paused = true;
  await captured.frame(0);
  assert.ok(
    captured.translationRequests().length > 0,
    "video pause starts cache work when a full track is available",
  );
  const stopped = await captured.pageMessage({ type: "GET_PAGE_STATUS" });
  assert.match(
    stopped.status.lastError,
    /provider returned no translations/,
    "a no-progress provider response stops instead of retrying the same paused batch forever",
  );
}

// The explicit translation pause stops provider work by design and says that
// it also stops background caching.
{
  const paused = createHarness(CUE_TRACK, {
    site: "netflix",
    settings: { translationPaused: true },
  });
  await paused.start({ captureTrack: true });
  paused.video.paused = true;
  await paused.frame(0);
  assert.deepEqual(paused.translationRequests(), []);
  const status = await paused.pageMessage({ type: "GET_PAGE_STATUS" });
  assert.equal(status.status.pausedCacheReason, "translation-paused");
  assert.match(status.status.pausedCacheBlocker, /caching is paused too/);
}

// --- probe ---
{
  const probe = createHarness(CUE_TRACK, {
    site: "primevideo",
    player: { mounted: false, started: false },
  });
  const report = (label) => {
    const pill = probe.elementById("lst-quick-pills");
    console.error(label, JSON.stringify({
      pill: Boolean(pill),
      display: pill?.style?.display,
      state: pill?.queryCache?.get("#lst-pill-state")?.textContent,
      sent: probe.sentMessageTypes(),
      events: probe.debugEvents().map((e) => e.event),
    }));
  };
  await probe.start({ captureTrack: false });
  report("1. document_start, no player:");
  await probe.identity({
    reason: "catalog-metadata",
    catalogSource: "catalogMetadata",
    itemType: "EPISODE",
    title: "The Smile",
    episodic: true,
    showName: "Homeland",
    showNameSource: "series-ancestor",
    videoId: "amzn1.dv.gti.e4b2f72f-8a6e-405e-44fe-bedba416d622",
    videoIdSource: "catalog-id",
    seasonNumber: 2,
    episodeNumber: 1,
  });
  report("1b. after the listing identity arrived:");
  probe.mountPlayer();
  await probe.routeTick();
  report("3. after the player mounts + route poll:");
  await probe.frame(1);
  report("4. one playback frame:");
  await probe.capture(CUE_TRACK);
  await probe.frame(1);
  report("5. after a captured track:");
}
