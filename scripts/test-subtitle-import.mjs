...
assert.ok(
  backgroundScripts.indexOf("subtitle-import.js") > backgroundScripts.indexOf("episode-identity.js"),
  "background.js must load after the identity module it reads",
);
assert.ok(
  backgroundScripts.indexOf("subtitle-import.js") < backgroundScripts.indexOf("background.js"),
  "subtitle-import.js must load before background.js",
);
const pageScripts = manifest.content_scripts[1].js;
assert.ok(
  pageScripts.includes("subtitle-import.js"),
  "the content script must load the import module",
);
assert.ok(
  pageScripts.indexOf("subtitle-import.js") < pageScripts.indexOf("content.js"),
  "subtitle-import.js must load before content.js",
);
assert.ok(
  pageScripts.indexOf("subtitle-import.js") > pageScripts.indexOf("episode-identity.js"),
...
for (const page of ["options.html", "popup.html"]) {
  ...
  assert.ok(
    /<script src="subtitle-import\.js"><\/script>/.test(html),
    `${page} must load subtitle-import.js`,
  );
  assert.ok(
    html.indexOf('<script src="episode-identity.js">') <
      html.indexOf('<script src="subtitle-import.js">'),
    `${page} must load the identity module first`,
  );
}

const packageJson = JSON.parse(await read("package.json"));
assert.match(packageJson.scripts["check:syntax"], /node --check subtitle-import\.js/);
assert.equal(packageJson.scripts["test:subtitle-import"], "node scripts/test-subtitle-import.mjs");
...
assert.match(prepareBrowser, /"subtitle-import\.js"/);
const verifyPackage = await read("scripts/verify-package.mjs");
assert.match(verifyPackage, /"subtitle-import\.js"/);
...
assert.match(backgroundSource, /importScripts\([\s\S]{0,300}"subtitle-import\.js"/);
assert.ok(
